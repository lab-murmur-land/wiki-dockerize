...
install-completion)
	...
	cat >"$COMPLETION_DIR/manage.bash" <<'EOF'
_manage_bash_completion() {
    local cur="${COMP_WORDS[COMP_CWORD]}"
    local prev="${COMP_WORDS[COMP_CWORD-1]}"
    local commands="up down build clean init logs wiki_sh wiki_cli restart restart-wiki reload reinstall dump-db import-db list-backups compose dev-init fix-images-permissions help"
    ...
    case "${prev}" in
        import-db)
            ...
            ;;
        wiki_cli)
            COMPREPLY=( $(compgen -W "init-db composer-install" -- "${cur}") )
            ;;
        *)
...
complete -F _manage_bash_completion manage.bash
complete -F _manage_bash_completion ./manage.bash
EOF
	echo "Completion kuruldu. 'exec bash' ile aktif et."
	;;
help | *)
